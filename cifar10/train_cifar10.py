...
try:
    ...
    wandb = None
    print0("WandB not available—continuing without experiment logging.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run", type=str, default='default_run3')
    ...
    print(f"cfg.data.name: {cfg.data.name}")
    print(f"model_cfg_name: {model_cfg_name}")
    print(f"args.run: {args.run}")

    # file_name = results/{cfg.data.name}/{model_cfg_name}
    logdir = os.path.join(f"/scratch/gpfs/cz5047/dxmi_results/{cfg.data.name}/{model_cfg_name}", args.run)
    if local_rank == 0:
        ...
        logger = BaseLogger(writer, use_wandb=False)
    ...
    if cfg.data.name == "cifar10":
        ...
        raise NotImplementedError
    m2, s2 = d_fid_stats["mu"], d_fid_stats["sigma"]
